export default [
    {
        'name': 'hardVeg',
        'placeholder': 'Choose vegetable one...',
        'value': [
            {
                'id': 'kjl234908fw',
                'name': 'Broccoli',
                'preparation': 'Preheat oven to 425°F, toss cut broccoli with oil to coat and salt to taste, arrange florets in a single layer on a oiled or parchment lined backing sheet, roast for 16-20 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '425°F',
                'time': '16-20 min.',
                'image': '../assets/broccoli.jpg',
                'image2': '../assets/roastedBroccoli.jpg'
            },
            {
                'id': '239084lkjwnef98',
                'name': 'Cauliflower',
                'preparation': 'Preheat oven to 425°F, toss cut cauliflower with oil to coat and salt to taste, arrange florets in a single layer on a oiled or parchment lined backing sheet, roast for 40-45 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '425°F',
                'time': '40-45 min.',
                'image': '../assets/cauliflower.jpg',
                'image2': '../assets/roastedCauliflower.jpg'

            },
            {
                'id': 'we09rukjnwefui',
                'name': 'Brussels Sprouts',
                'preparation': 'Preheat oven to 400°F, toss cut brussels sprouts with oil to coat and salt to taste, arrange sprouts cut side down in a single layer on a oiled or parchment lined backing sheet, roast for 25 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '400°F',
                'time': '25 min.',
                'image': '../assets/brussels.jpg',
                'image2': '../assets/roastedBrussels.jpg'
            },
            {
                'id': 'fwnbekjhrfw89fwekj',
                'name': 'Potatoes',
                'preparation': 'Preheat oven to 375°F, cut potatoes into bite size chunks, toss with oil to coat and salt to taste, arrange potatoes in a single layer on a oiled or parchment lined backing sheet, roast for 35-45 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '375°F',
                'time': '35-45 min.',
                'image': '../assets/potatoes.jpg',
                'image2': '../assets/roastedPotatoes.jpg'
            },
            {
                'id': '089uwejknbr78',
                'name': 'Beet',
                'preparation': 'Preheat oven to 400°F, wash and scrub beets well, cut off the leafy tops and wrap each beet in aluminum foil, place the beets in a backing dish and depending on the size roast for an hour to an hour and 45 minutes or until done, let cool for 10-15 minutes, then wipe off the skin using paper towels. Cut beets into bite size pieces, drizzle with olive oil and salt to taste.',
                'prepShort': 'roasted',
                'temp': '400°F',
                'time': '1-1.75 hrs.',
                'image': '../assets/beet.jpg',
                'image2': '../assets/roastedBeets.jpg'
            },
            {
                'id': 'hsdf877ehuwe687',
                'name': 'Sweet Potatoes',
                'preparation': 'Preheat oven to 425°F, cut sweet potatoes into bite size chunks, toss with oil to coat and salt to taste, arrange sweet potatoes in a single layer on a oiled or parchment lined backing sheet, roast for 35-45 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '425°F',
                'time': '35-45 min.',
                'image': '../assets/sweetPotatoes.jpg',
                'image2': '../assets/roastedSweetPotatoes.jpg'
            },
            {
                'id': '1223190bnmders',
                'name': 'Onions',
                'preparation': 'Preheat oven to 400°F, cut onions into bite size pieces, toss with oil to coat and salt to taste, arrange onions in a single layer on a oiled or parchment lined backing sheet, roast for 25-30 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '400°F',
                'time': '25-30 min.',
                'image': '../assets/redAndWhiteOnions.jpg',
                'image2': '../assets/roastedOnions.jpg'
            },
            {
                'id': '890789657345bhjvoip',
                'name': 'Zucchini',
                'preparation': 'Preheat oven to 425°F, cut zucchini into 1/2in thick half moons, toss with oil to coat and salt to taste, arrange zucchini in a single layer on a oiled or parchment lined backing sheet, roast for 10-15 minutes or until done.',
                'prepShort': 'roasted',
                'temp': '425°F',
                'time': '10-15 min.',
                'image': '../assets/zucchini.jpg',
                'image2': '../assets/roastedZucchini.jpg'
            },
            {
                'id': '832hjkwefi67t',
                'name': 'Corn',
                'preparation': 'Preheat oven to 400°F, shuck the corn and remove as much of the silk as possible, toss with oil to coat and salt to taste, arrange the cobs on an aluminum lined baking dish, roast for 35-45 minutes or until done, finish under the broiler for a nice char. Cut the kernels of the cob over a large bowl to a void mess.',
                'prepShort': 'roasted',
                'temp': '400°F',
                'time': '35-45 min.',
                'image': '../assets/corn.jpg',
                'image2': '../assets/roastedCorn.jpg'
            },
            {
                'id': 'klj453bnyusddsiu89',
                'name': 'Butternut Squash',
                'preparation': 'Preheat oven to 425°F. Peel the squash, cut it in half, and scoop out all the seeds, then slice it into bit sized pieces and toss with oil and salt to taste. Place the cut squash on a oiled or aluminum foil lined baking sheet in a single layer, make sure not to overcrowd! the squash is done when it can easily be pierced by a fork. ',
                'prepShort': 'roasted',
                'temp': '425°F',
                'time': '40-50 min.',
                'image': '../assets/butternutSquash.jpg',
            },
            {
                'id': 'kjhgjds675iouerojmnbvcxz',
                'name': 'Eggplant',
                'preparation': 'Preheat oven to 400°F. Pierce the skin in several places. Roast on a baking sheet until soft, 35 to 45 minutes. Remove from oven, and let cool. Slice eggplants open, and scoop out the flesh',
                'prepShort': 'roasted',
                'temp': '400°F',
                'time': '35-45 min.',
                'image': '../assets/eggplant.jpg',
            },
        ]
    },
    {
        'name': 'softVeg',
        'placeholder': 'Choose vegetable two...',
        'value': [
            {
                'id': '178hjkdfsg879uyihjk',
                'name': 'Onion',
                'preparation': 'Finely slice two onions in 1/8 strips. Place onions in a medium heatproof bowl. Bring 1 cup of vinegar, 2 tablespoons of sugar, and 1 tablespoon of salt to a boil in a saucepan; pour over onions. Let cool completely. Refrigerate, covered, for at least 2 hours.',
                'prepShort': 'pickled',
                'temp': null,
                'time': '2 1/2 hours',
                'image': '../assets/redAndWhiteOnions.jpg',
                'image2': '../assets/pickledRedOnions.jpg'
            },
            {
                'id': 'iuyf786786hgj',
                'name': 'Celery',
                'preparation': 'Slice the celery on a bias into 1/4 in strips.',
                'prepShort': 'sliced',
                'temp': null,
                'time': null,
                'image': '../assets/celery.jpg',
                'image2': '../assets/slicedCelery.jpg'
            },
            {
                'id': '1njkijh897hjkfr',
                'name': 'Spinach',
                'preparation': 'Wash spinach and de-stem if necessary.',
                'prepShort': 'raw',
                'temp': null,
                'time': null,
                'image': '../assets/spinach.jpg'
            },
            {
                'id': '78943jkhdfs675675',
                'name': 'Kale',
                'preparation': 'Bring a pot of salted water to a rolling boil, while waiting, prepare an ice bath. Wash kale, trim the kale stems and break down the kale into bite sized pieces. Blanch the kale for 2 minutes, then shock the kale in the ice bath for 1 minute. Remove from the ice bath and pat dry with paper towels.',
                'prepShort': 'blanched',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/kale.png'
            },
            {
                'id': '1jhkewiouuiyerw231gh',
                'name': 'Bell Pepper',
                'preparation': 'Wash, slice, and de-seed bell peppers.',
                'prepShort': 'sliced',
                'temp': null,
                'time': null,
                'image': '../assets/bellPeppers.jpg'
            },
            {
                'id': 'jner6767tjkn',
                'name': 'Cucumber',
                'preparation': 'Wash, and peel if desired, slice into half moons.',
                'prepShort': 'sliced',
                'temp': null,
                'time': null,
                'image': '../assets/cucumber.jpg'
            },
            {
                'id': '16knjefrhjewfty789u',
                'name': 'Green Beans',
                'preparation': 'Bring a pot of salted water to a rolling boil, while waiting, prepare an ice bath. Wash green beans, and trim the stems. Blanch the green beans for 2 minutes, then shock the green beans in the ice bath for 1 minute. Remove from the ice bath and pat dry with paper towels.',
                'prepShort': 'blanched',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/greenBeans.jpg'
            },
            {
                'id': 'mndsf67dfsuifs78y',
                'name': 'Tomatoes',
                'preparation': 'Wash and de-stem tomatoes',
                'prepShort': 'raw',
                'temp': null,
                'time': null,
                'image': '../assets/tomatoes.jpg'
            },
            {
                'id': '18njkjknfweioio',
                'name': 'Mushrooms',
                'preparation': 'Slice mushrooms into 1/4 in pieces, depending on the type and size you may also need to cut them in half. Bring a large saute pan to temp over medium-high heat. Add just enough oil to coat the bottom of the pan. Saute the mushrooms for 4-5 minutes stirring occasionally, add a couple glugs of vegetable or chicken stock to the pan and cover with a lid. Reduce the heat to medium low and cook for another 4 minutes stirring occasionally, season to taste.',
                'prepShort': 'sauted',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/mushrooms.jpg',
                'image2': 'assets/sautedMushrooms.jpg'
            },
            {
                'id': '19jklefriurf789',
                'name': 'Lettuce',
                'preparation': 'Wash and trim lettuce',
                'prepShort': 'raw',
                'temp': null,
                'time': null,
                'image': '../assets/lettuce.jpg'
            },
            {
                'id': 'kljk221367sdfbnvfg',
                'name': 'Arugula',
                'preparation': 'If its not pre-washed, give it a quick wash.',
                'prepShort': 'raw',
                'temp': null,
                'time': null,
                'image': '..assets/arugula.png'
            },
        ]
    },
    {
        'name': 'starch',
        'placeholder': 'Choose your starch...',
        'value': [
            {
                'id': '20erklreg5623uyi',
                'name': 'White Rice',
                'preparation': 'Wash the rice three times. if you have a rice cooker (you should) put the rice in it, if not place it in a sauce pan and cover with equal parts water(1:1). Bring to a boil, stir, reduce to a low simmer and cover with a lid for 15 minutes before checking then fluff with a fork and serve.',
                'prepShort': '',
                'temp': null,
                'time': '15 min.',
                'image': '../assets/rice.jpg'
            },
            {
                'id': 'hiow890edg',
                'name': 'Brown Rice',
                'preparation': 'Wash the rice three times. if you have a rice cooker (you should) put the rice in it, if not place it in a sauce pan and cover with 2 1/2 parts water to brown rice. Bring to a boil, stir, reduce to a low simmer and cover with a lid for 40 to 45 minutes before checking then fluff with a fork and serve.',
                'prepShort': '',
                'temp': null,
                'time': '40-45 min.',
                'image': '../assets/brownRice.jpg'
            },
            {
                'id': '21lkfiodfsy78hj',
                'name': 'Barley',
                'preparation': 'Wash the barley. In a medium saucepan, mix together one cup dry barley with three cups of water/stock and a hearty pinch of salt. Bring the pot to a boil, keeping an eye on it because it may become foamy at first and boil over. Then reduce to a simmer and cook, adding more water if the pan dries out, until done with a chewy but tender texture. For pearl barley this will take about 25 to 30 minutes and hulled barley 40 to 50 minutes. Drain any excess water that may be left, then fluff with a fork and serve.  ',
                'prepShort': '',
                'temp': null,
                'time': '25-30 min. or 40-50 min.',
                'image': '../assets/barley.jpg'
            },
            {
                'id': '22fdjklsdfuyg43',
                'name': 'Quinoa',
                'preparation': 'Wash the quinoa well with cold water. If you have a rice cooker (you should) use it, if not place the quinoa in a sauce pan and cover with twice the amount of water or stock (2:1). Bring to a boil, stir, reduce to a low simmer and cover with a lid for 15 minutes before checking, if the water has evaporated remove from the heat and let sit for 5 minutes then fluff with a fork and serve.',
                'prepShort': '',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/quinoa.jpg'
            },
            {
                'id': 'lkjhgfdsawerty9876543cvb',
                'name': 'Freekeh',
                'preparation': 'For every cup of freekeh bring 2 1/2 cups water to a boil. Add freekeh and 1/2 teaspoon salt. Reduce heat to medium-low and simmer, covered, until freekeh is tender and liquid is absorbed, about 20 minutes.Remove from heat and let stand, covered, 5 minutes. Use immediately or spread on a rimmed baking sheet and let cool.',
                'prepShort': '',
                'temp': null,
                'time': '25 min.',
                'image': '../assets/freekeh.jpg'
            },
            {
                'id': 'qwertyu9876543xcvbnjhgfla1',
                'name': 'Wheat Berries',
                'preparation': 'Place wheat berries in a small stockpot over high heat. Add water. Cover; bring to a boil. Reduce heat to low. Simmer until tender, at least 40 minutes. Drain.',
                'prepShort': '',
                'temp': null,
                'time': '40-60 min.',
                'image': '../assets/wheatBerries.jpg'
            }
        ]
    },
    {
        'name': 'protein',
        'placeholder': 'Choose your protein...',
        'value': [
            {
                'id': '23sfdkmnsdfgy45tfybhjio',
                'name': 'Chickpeas',
                'preparation': 'Wash and sort the chickpeas to make sure there are no stones or debris. Cover the chickpeas in cold water and soak over night or for at least 12 hours. Drain and wash the chickpeas and transfer them to a pot- cover with cold water twice the amount of chickpeas and bring to a boil. Cover, lower the heat and let simmer for 60 minutes or until they are done to your liking. Drain and let cool for at least 15 minutes.',
                'temp': null,
                'time': '14 hrs.',
                'prepShort': '',
                'image': '../assets/chickpeas.jpg'
            },
            {
                'id': '2jkfrhdfs6778yhj4',
                'name': 'Green or Brown Lentils',
                'preparation': 'Wash and sort the lentils to make sure there are no stones or debris. Transfer them to a pot- cover with stock or water twice the amount of the lentils and bring to a boil. Cover, lower the heat and let simmer for 25-30 minutes or until they are done to your liking. Drain and season with salt and pepper to taste.',
                'prepShort': '',
                'temp': null,
                'time': '40 mins.',
                'image': '../assets/greenLentils.jpg'
            },
            {
                'id': '098765432zxcvbnmghtyr',
                'name': 'Red and Orange Lentils',
                'preparation': 'Wash and sort the lentils to make sure there are no stones or debris. Transfer them to a pot- cover with stock or water twice the amount of the lentils and bring to a boil. Cover, lower the heat and let simmer for 20 minutes or until they are done to your liking. Drain and season with salt and pepper to taste.',
                'prepShort': '',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/redLentils.jpg'
            },
            {
                'id': '2dfjioefr8y7jkio5',
                'name': 'Black Beans',
                'preparation': 'Wash and sort the black beans to make sure there are no stones or debris. Cover the black beans in cold water and soak over night or at least 12 hours. Drain and wash the black beans and transfer them to a pot- cover with cold  water twice the amount of the black beans and bring to a boil. Cover, lower the heat and let simmer for around 2 hours stirring periodically.You may need to top off the pot with boiling water due to evaporation. Drain and season with salt and pepper to taste.',
                'prepShort': '',
                'temp': null,
                'time': '14 hrs.',
                'image': '../assets/blackBeans.jpg'
            },
            {
                'id': '2hjkf544532jkn6',
                'name': 'Tofu',
                'preparation': 'Preheat the oven to 350°F. Drain tofu and place on a plate. Cover with another plate and weight the top with a food can or other object of about 1 pound. Let stand 20 to 30 minutes to extract excess liquid. Pour off liquid and cut tofu into 3/4 -inch-thick slices. Place slices in a single layer in a shallow dish. Dress the tofu with your your favorite seasoning or marinade  and let sit for 30 minutes.  Line a large baking sheet with parchment paper. Remove tofu from the marinade and place on the baking sheet. Bake until firm and lightly browned, about 1 hour.',
                'prepShort': 'roasted',
                'temp': '350°F',
                'time': '60 min.',
                'image': '../assets/tofu.jpg',
                'image2': '../assets/roastedTofu.jpg'
            },
            {
                'id': '2mdfgtr5643wioujkh7',
                'name': 'Chicken',
                'preparation': `First you want to truss the chicken: cut about a 3-foot length of heatproof butcher's twine. Lay the chicken on a clean surface with the breast facing up. Hold one end of the string in each hand, and loop the center of the string underneath the chicken's tail. Catch the ends of the legs inside the string, then cross the string over the chicken's breast, making an X. Loop the string under and around the wings, then tie the string snugly in a knot across the middle of the breast. Make sure that the ends of the wings are tucked in. Now to dry-brine the chicken: Measure 1 tablespoon kosher salt (not table salt) for every 5 pounds of chicken. You can add dry spices or lemon zest if you'd like, but it's the salt that will make the chicken juicy. Sprinkle the salt inside and outside the chicken. Cover the chicken loosely with plastic wrap and refrigerate for a few hours or up to 3 days. If you refrigerate for 3 days, uncover the chicken during the last 8 hours so the skin can air dry. This creates the crispiest skin. No need to rinse the bird, simply pat dry as needed and proceed with the recipe of your choice. Just remember the chicken drippings will be salty, so taste them before adding any more seasoning to the pan sauce or gravy. Now lets cook the bird: Preheat oven to 450°F and cook whole chicken for 10-15 minutes. Then reduce the temperature to 350°F and roast for 20 minutes per pound. Let Rest for at least 15 minutes before carving.`,
                'prepShort': 'roasted',
                'temp': '450°F - 350°F',
                'time': '10-15 min. & 20min. per lb.',
                'image': '../assets/wholeChicken.png',
                'image2': '../assets/roastedWholeChicken.jpg'
            },
            {
                'id': '2frjklfdsiotyferw5648',
                'name': 'Beef',
                'preparation': `Preheat Oven to 360°F. Brown the beef chunks in batches in a crock pot filled with hot oil. Add 1 medium sized onion, diced, to the pot and saute till translucent (1 onion per 1 1/2 lb of meat). Return the browned beef chunks to the pot add salt, pepper, a couple cloves of pressed garlic, and any other seasonings of your liking. Add enough water/stock  to cover beef by 1inch, then cover with a lid and bring to a boil on your cooktop. Then place in the oven and cook for 1 and a half hours or until the the beef is falling apart.`,
                'prepShort': 'braised',
                'temp': '360°F',
                'time': '60-90 min.',
                'image': '../assets/beefChuckRaw.jpg',
                'image2': '../assets/braisedBeef.png'
            },
            {
                'id': '2mnkdf2334890hj9',
                'name': 'Eggs',
                'preparation': `Place eggs in a large saucepan. Cover them with cool water by 1 inch. Slowly bring water to a boil over medium heat; when the water has reached a boil, cover and remove from heat. Let sit 12 minutes. Transfer eggs to a colander; place under cool running water to stop the cooking. Eggs can be peeled and served immediately.`,
                'prepShort': 'hard boiled',
                'temp': null,
                'time': '20 min.',
                'image': '../assets/eggs.jpg',
                'image2': '../assets/hardBoiledEgg.jpg'
            },
            {
                'id': '78392hiouqpmzt67',
                'name': 'Salmon',
                'preparation': `Let salmon come to room temperature, about 20-30 min. Preheat oven to 450°F. Blot the salmon dry on all sides and season with salt. Then place skin side down on an oiled oven proof skillet or an aluminum foil lined baking sheet and bake salmon for 10-15 min depending on the size or till it reaches an internal temp of 140°F or the salmon flakes easily when pierced with a fork. Squeeze with with lemon and enjoy!`,
                'prepShort': 'Roasted',
                'temp': '450°F',
                'time': '35-45 min.',
                'image': '../assets/salmon.jpg',
                
            }
        ]
    }
];

export const sauces = [
    [
        {
            name: 'Peanut Sauce',
            id: 'peanut',
            category: 1,
            herb: 'cilantro',
            acid: 'lime juice',
            shopping: ['Cilantro', 'Creamy Peanut Butter', 'Tamari', ' Maple Syrup', 'Chili Garlic Sambal', 'Lime'],
            ingredients: ['1/2 cup creamy peanut butter', '2 Tbsp tamari', '2 Tbsp maple syrup', '1 tsp Sambal', '3 Tbsp lime juice', '1/4 cup water'],
            preparation: 'Combine all ingredients besides water in a medium sized mixing bowl and whisk until incorporated. Add water until sauce is the desired consistency.'
        },
        {
            name: 'Coconut Sweet Chili Stir Fry Sauce',
            id: 'coconut',
            category: 1,
            herb: 'thai basil',
            acid: 'lime juice',
            shopping: ['Thai Basil', 'Coconut Milk', 'Sweet Chili Sauce', 'Soy Sauce', 'Lime', 'Red Pepper Flakes'],
            ingredients: ['1/2 can of coconut milk', '1/4 cup of sweet chili sauce', '2 Tbsp soy sauce', '2 Tsp lime juice', '1/2 Tsp red pepper flakes'],
            preparation: 'Combine all ingredients in a container with a lid and shake until incorporated.'
        },
        {
            name: 'Ponzu Sauce',
            id: 'ponzu',
            category: 1,
            herb: 'green onions',
            acid: 'lemon juice',
            shopping: ['Green Onions', 'Soy Sauce', 'Lemon', 'Lime', 'Mirin', 'Rice Vinegar', 'Bonito Flakes(optional)'],
            ingredients: ['1/2 cup soy sauce', '1/4 cup lemon juice', '1/4 cup lime juice', '2 Tbsp mirin', '1 Tbsp rice vinegar', '1/3 cup bonito flakes(optional)'],
            preparation: 'Combine all the ingredients in a storage container, stir, cover and refrigerate. Let sit for at least 3 hours or overnight. Strain before use.'
        },
        {
            name: 'Ssamjang',
            id: 'ssamjang',
            category: 1,
            herb: 'green onions',
            acid: 'sesame oil',
            shopping: ['Green Onions', 'Doenjang(fermented soybean paste)', 'Gochujang(hot pepper paste)', 'Garlic', 'Scallions', 'Sesame Oil', 'Sesame Seeds', 'Honey'],
            ingredients: ['1/2 cup doenjang(Korean fermented soybean paste)', '3 Tbsp gochujang(Korean hot pepper paste)', '2 large garlic cloves', '2 scallions', '1 1/2 Tbsp toasted sesame oil', '2 Tsp sesame seeds', '2 Tbsp honey'],
            preparation: 'Place all ingredients in a large bowl and mix well.'
        }

    ],
    [
        {
            name: 'Cilantro Lime Yogurt Sauce',
            id: 'yogurt',
            category: 2,
            herb: 'cilantro',
            acid: 'lime juice',
            shopping: ['Greek Yogurt', 'Cilantro', 'Lime'],
            ingredients: ['1 cup plain Greek yogurt', '1/4 cup chopped cilantro', '1 lime, juiced', '1/2 Tsp salt'],
            preparation: 'Stir ingredients together in a bowl until smooth. Cover with plastic wrap and refrigerate for at least 20 minutes before use.'
        },
        {
            name: 'Chimichurri Sauce',
            id: 'chimichurri',
            category: 2,
            herb: 'parsley',
            acid: 'red wine vinegar',
            shopping: ['Flat Leaf Parsley', 'Garlic', 'Fresh Oregano', 'Olive Oil', 'Red Wine Vinegar', 'Black Pepper', 'Red Pepper Flakes'],
            ingredients: ['1 firmly packed cup of flat leaf parsley leaves', '3-4 garlic cloves', '2 Tbsp fresh oregano leaves', '1/3 cup virgin olive oil', '2 Tbsp red wine vinegar', '1/2 Tsp salt', '1/8 Tsp ground black pepper', '1/4 Tsp red pepper flakes'],
            preparation: 'Finely chop parsley, oregano, and garlic. Place in a small bowl. Stir in olive oil, vinegar, salt, pepper and red pepper flakes. adjust seasonings to taste. Serve at room temperature.'
        },
        {
            name: 'Vegan Chipotle Aioli',
            id: 'chipotle',
            category: 2,
            herb: 'cilantro',
            acid: 'lemon juice',
            shopping: ['Cilantro', 'Cashews', 'Lemon', 'Maple Syrup', 'Chipotle Peppers in Adobo Sauce', 'Avocado Oil(or any other vegetable oil)', 'Smoked Paprika'],
            ingredients: ['3/4 cup raw cashews', '1/2 cup water', '2 Tbsp lemon juice', '1/4 Tsp salt', '2 Tsp maple syrup(or sub agave, cane sugar or stevia)', '2-3 whole chipotle peppers in adobo sauce', '2 Tbsp avocado oil(or sub any other vegetable oil)', '1 pinch smoked paprika'],
            preparation: 'Soak cashews in boiling hot water for one hour. Drain cashews and add to a blender with water, lemon juice, salt, maple syrup and peppers. Blend on high, adding the oil and adding water until it incorporates easily. Season to taste with salt, paprika and lemon.'
        },
        {
            name: 'Sesame Miso Sauce',
            id: 'sesame',
            category: 2,
            herb: 'green onions',
            acid: 'lime juice',
            shopping: ['Green Onions', 'Sesame Oil', 'Miso', 'Soy Sauce', 'Maple Syrup', 'Apple Cider Vinegar', 'Sesame Seeds', 'Ginger', 'Lime'],
            ingredients: ['3 Tbsp toasted sesame oil', '2 Tbsp miso', '2 Tbsp water', '1 Tbsp soy sauce', '1 Tbsp maple syrup', '1 Tbsp apple cider vinegar', '1 Tbsp sesame seeds', '1" ginger. grated', 'juice of one lime'],
            preparation: 'Combine all ingredients in a container with a sealed lid and shake vigorously until well combined. Refrigerate until use.'
        }
    ],
    [
        {
            name: 'Balsamic Vinaigrette',
            id: 'balsamic',
            category: 3,
            herb: 'tarragon',
            acid: 'balsamic vinegar',
            shopping: ['Tarragon', 'Olive Oil', 'Balsamic Vinegar', 'Dijon Mustard', 'Maple Syrup', 'Garlic', 'Black Pepper'],
            ingredients: ['1⁄2 cup extra-virgin olive oil', '3 Tbsp balsamic vinegar', '1 Tbsp Dijon mustard', '1 Tbsp maple syrup', '2 medium cloves garlic, minced', '1⁄4 Tsp fine sea salt, to taste', 'Freshly ground black pepper, to taste'],
            preparation: 'In a liquid measuring cup or bowl, combine all of the ingredients. Stir well with a small whisk or a fork until the ingredients are completely mixed together. Serve immediately. Cover and refrigerate the rest for future use. Keeps well for 7 to 10 days. After refrigeration, remove 30 minutes prior to using, and shake to combine.'
        },
        {
            name: 'Italian Vinaigrette',
            id: 'italian',
            category: 3,
            herb: 'parsley',
            acid: 'red wine vinegar',
            shopping: ['Flat Leaf Parsley', 'Garlic', 'Olive Oil', 'Red Wine Vinegar', 'Lemon', 'Oregano', 'Dijon Mustard', 'Sugar', 'Black Pepper'],
            ingredients: ['4 medium cloves garlic, minced', '1⁄4 cup olive oil', '1⁄4 cup red wine vinegar', '2 Tbsp lemon juice', '2 Tbsp dried oregano', '1 Tbsp dijon mustard', '1 1⁄2 Tsp kosher salt', '1 Tsp sugar', '1/4 Tsp freshly ground black pepper'],
            preparation: 'In a liquid measuring cup or bowl, combine all of the ingredients. Stir well with a small whisk or a fork until the ingredients are completely mixed together. Serve immediately. Cover and refrigerate the rest for future use. Keeps for 7 to 10 days. After refrigeration, remove 30 minutes prior to using, and shake to combine.'
        },
        {
            name: 'Creamy Honey Mustard Dressing',
            id: 'honey',
            category: 3,
            herb: 'dill',
            acid: 'lemon juice',
            shopping: ['Fresh Dill', 'Greek Yogurt', 'Olive Oil', 'Dijon Mustard', 'Honey', 'Lemon', 'Apple Cider Vinegar', 'Garlic', 'Black Pepper'],
            ingredients: ['1⁄2 cup plain Greek yogurt (any % fat will do)', '1⁄4 cup extra-virgin olive oil', '1⁄4 cup Dijon mustard', '3 to 4 Tbsp honey', '2 Tbsp lemon juice', '2 Tbsp apple cider vinegar', '1 clove garlic, minced', '1⁄2 Tsp fine sea salt', 'freshly ground black pepper to taste'],
            preparation: 'In a  bowl, combine all of the ingredients as listed. Whisk until blended. Taste, and season with additional pepper if necessary. This dressing is intentionally bold, but if it tastes too tart for your liking, whisk in another tablespoon of honey.Store leftover salad dressing in the refrigerator, covered, for 10 to 14 days. Shake to combine after removing from fridge.'
        },
        {
            name: 'Zesty Tahini Dressing',
            id: 'tahini',
            category: 3,
            herb: 'cilantro',
            acid: 'apple cider vinegar',
            shopping: ['Cilantro', 'Tahini', 'Apple Cider Vinegar', 'Lemon', 'Tamari', 'Nutritional Yeast', 'Garlic'],
            ingredients: ['1/4 cup tahini', '1/4 cup apple cider vinegar', '1/4 cup lemon juice', '1/4 cup tamari or soy sauce', '1/2 cup nutritional yeast', '1 Tbsp minced garlic'],
            preparation: 'Add all ingredients into a blender and blend until combined. Store leftover salad dressing in the refrigerator, covered, for 10 to 14 days. Shake to combine after removing from fridge.'
        }
    ]
];